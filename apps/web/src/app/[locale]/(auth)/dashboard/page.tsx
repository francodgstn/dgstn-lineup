...
import {
  ...
  getDocs, Timestamp, collectionGroup,
...
import type { Contact, Session, Booking, RankingSystem } from '@lineup/shared'
...
function StatCard({ title, value, subtitle, icon: Icon, loading, href }: {
    ...
    <Link href={`/sessions/${session.id}` as Route}
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {(session.participants_count ?? 0) > 0 && (
          <span className="text-xs text-muted-foreground">{t('attended', { count: session.participants_count ?? 0 })}</span>
        )}
      </div>
    </Link>
  )
...
function QuickActionsCard({ teamSlug }: { teamSlug?: string }) {
  const t = useTranslations('Dashboard')
  ...
  return (
    <Card>
      <CardHeader className="pb-2"><CardTitle className="text-base">{t('quickActions')}</CardTitle></CardHeader>
      <CardContent>
...
function RecentBookingsCard({ teamId }: { teamId: string | null }) {
          ...
          <div className="space-y-2.5">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <div className="flex-1 space-y-1"><Skeleton className="h-3.5 w-28" /><Skeleton className="h-3 w-16" /></div>
              </div>
            ))}
          </div>
        ) : !bookings?.length ? (
            ...
            {bookings.map((b) => (
              <div key={b.id} className="flex items-center gap-2.5">
                <div className="h-7 w-7 rounded-full bg-muted flex items-center justify-center flex-shrink-0">
                  <span className="text-xs font-semibold text-muted-foreground">{(b.firstname?.[0] ?? '?').toUpperCase()}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{b.firstname} {b.lastname}</p>
                  <p className="text-xs text-muted-foreground">{b.email}</p>
                </div>
                {b.is_new_contact && <Badge variant="outline" className="text-xs flex-shrink-0">{t('trialBadge')}</Badge>}
              </div>
            ))}
          </div>
        )}
...
export default function DashboardPage() {
  ...
  const upcomingBookingsCount = sessions?.reduce(
    (sum, s) => sum + ((s as { portal_bookings_count?: number }).portal_bookings_count ?? 0), 0,
  ) ?? null
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── 1. Agenda + quick actions ── */}
      <section className="space-y-3">
        <SectionHeading>{t('sectionAgenda')}</SectionHeading>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          <div className="lg:col-span-7"><AgendaCard teamId={currentTeamId} /></div>
          <div className="lg:col-span-5 space-y-4">
            <QuickActionsCard teamSlug={teamSlug} />
            <RecentBookingsCard teamId={currentTeamId} />
          </div>
        </div>
      </section>
