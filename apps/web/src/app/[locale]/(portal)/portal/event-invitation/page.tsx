...
export default function EventInvitationPage() {
  ...
  useEffect(() => {
    ...
    const fn = httpsCallable<
      { token: string; trackView: boolean },
      InvitationDetails
    >(functions, 'getEventInvitationDetails')
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center px-5 pt-8 pb-12">
        <div className="w-full max-w-md space-y-5">
          ...
          {/* ── Main content ─────────────────────────────────────────────────── */}
          {(pageState === 'ready' || pageState === 'confirmed-attend' || pageState === 'confirmed-decline') && details && (
            <>
              {/* Greeting */}
              ...
              {pageState === 'confirmed-attend' && (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-center space-y-2">
                  ...
                  <p className="font-semibold text-emerald-800">{t('invitation_confirmedAttending')}</p>
                </div>
              )}

              {pageState === 'confirmed-decline' && (
                <div className="rounded-2xl border bg-white p-6 text-center">
                  <p className="text-sm text-muted-foreground">{t('invitation_confirmedDeclined')}</p>
                </div>
              )}
                ...
                <div className="space-y-3">
                  ...
                  {rsvpStatus === 'attending' && (
                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-emerald-50 border border-emerald-200">
                      ...
                      <p className="text-sm text-emerald-800 font-medium">{t('invitation_alreadyAttending')}</p>
                    </div>
                  )}
                  {rsvpStatus === 'declined' && (
                    <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-muted border">
                      <p className="text-sm text-muted-foreground">{t('invitation_alreadyDeclined')}</p>
                    </div>
                  )}
