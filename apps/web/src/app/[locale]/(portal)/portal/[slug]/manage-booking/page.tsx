...
export default function ManageBookingPage() {
  ...
  const loadDetails = async (tok: string) => {
    ...
    setError(null)
    try {
      const fn = httpsCallable<{ token: string }, { success: boolean } & BookingDetails>(functions, 'getBookingDetails')
  ...
  return (
    <div className="space-y-6 max-w-lg">
      ...
      <div className="rounded-xl border bg-card p-5 space-y-4">
        <div>
          <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">{t('bookingFor')}</p>
      ...
      {canCancel && !showCancelConfirm && (
        <Button variant="outline" className="text-destructive border-destructive/30 hover:bg-destructive/5 w-full" onClick={() => setShowCancelConfirm(true)}>
          {t('cancelButton')}
        </Button>
      )}
      ...
      {canCancel && showCancelConfirm && (
        <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <Button variant="destructive" size="sm" onClick={handleCancel} disabled={cancelling} className="flex-1">
              {cancelling ? t('cancelling') : t('cancelConfirmYes')}
            </Button>
            <Button variant="outline" size="sm" onClick={() => setShowCancelConfirm(false)} disabled={cancelling} className="flex-1">
              {t('cancelConfirmNo')}
            </Button>
          </div>
        </div>
