...
import {
  ...
  isSameDay, isToday, isAfter, startOfDay, format,
} from 'date-fns'
...
function StickyBar({ activity, session, accentColor, showConfirm, submitting, onConfirm }: StickyBarProps) {
  const bg = activity.image
    ? `url("${activity.image}")`
    : activityGradient(activity.name)
        ...
        {session && (
          <div className="flex flex-col gap-0.5 mt-0.5">
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              <svg className="h-3 w-3 shrink-0 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" />
              </svg>
              <span>{formatDate(session.start)} · {formatTime(session.start)}–{formatTime(session.end)}</span>
            </div>
            {session.location && (
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <svg className="h-3 w-3 shrink-0 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export default function BookingForm({ slug, preSelectedActivitySlug, initialDate }: Props) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (teamSnap.empty) { setLoadingData(false); return }
        const teamDoc = teamSnap.docs[0]
        const resolvedTeamId = teamDoc.ref.parent.parent?.id
        if (!resolvedTeamId) { setLoadingData(false); return }
        ...
        const actList: ActivityProfile[] = actSnap.docs.map((d) => {
          const data = d.data()
          return {
            id:          d.id,
            name:        data.name || '',
  ...
  useEffect(() => {
    ...
    const candidate = initialDate && availableDates.includes(initialDate)
      ? initialDate
      : availableDates[0]
  ...
  const filteredSessions = useMemo(() =>
    selectedDate ? activitySessions.filter((s) => toDateKey(s.start) === selectedDate) : activitySessions,
  ...
  async function doAuthenticatedBooking(contactId: string, codeId: string, session: SessionProfile) {
    const bookSessionFn = httpsCallable(functions, 'bookSession')
    await bookSessionFn({ teamId, sessionId: session.id, authenticatedContactId: contactId, verificationCodeId: codeId })
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{ background: bg, backgroundSize: a.image ? 'cover' : '100% 100%', backgroundPosition: 'center' }}
                />
                {/* Content */}
                <div className="flex-1 p-4 min-w-0">
                  <div className="flex items-start gap-2 flex-wrap">
                    ...
                    {a.isFreeTrial && (
                      <span className="rounded-full bg-green-100 text-green-700 text-xs px-2 py-0.5 font-medium">Free Trial</span>
                    )}
                    {a.level && (
                      <span className="rounded-full bg-muted text-muted-foreground text-xs px-2 py-0.5">{a.level}</span>
                    )}
                    {!hasSessions && (
                      <span className="rounded-full bg-muted text-muted-foreground text-xs px-2 py-0.5">No open sessions</span>
                    )}
                  </div>
    ...
    return withBar(
      <>
        ...
        {availableDates.length === 0 ? (
          <div className="rounded-xl border bg-muted/30 p-8 text-center">
            <p className="text-muted-foreground text-sm">No sessions available for booking right now.</p>
          </div>
        ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        {!selectedActivity && s.activityName && (
                          <p className="text-xs font-medium text-muted-foreground mb-0.5">{s.activityName}</p>
                        )}
                        <p className="font-semibold text-sm">{formatTime(s.start)} – {formatTime(s.end)}</p>
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Verify with email and book instantly</p>
            </div>
            <svg className="h-4 w-4 text-muted-foreground group-hover:text-primary transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
            </svg>
    ...
    return withBar(
      <>
        ...
        <form onSubmit={emailForm.handleSubmit(onSendCode)} className="space-y-4">
          ...
          <PortalButton type="submit" disabled={emailForm.formState.isSubmitting} accentColor={accentColor}>
  ...
  if (step === 'ret-code' && selectedSession) {
    return withBar(
      <>
        <div>
          <BackButton onClick={() => { setStep('ret-email'); setReturningError(null); codeForm.reset() }} />
        ...
        <form onSubmit={codeForm.handleSubmit(onVerifyCode)} className="space-y-4">
          ...
          <PortalButton type="submit" disabled={codeForm.formState.isSubmitting} accentColor={accentColor}>
    ...
    return withBar(
      <>
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Just a few details and you&apos;re booked.</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              {guestForm.formState.errors.firstname && (
                <p className="text-xs text-destructive">{guestForm.formState.errors.firstname.message}</p>
              )}
            </div>
            <div className="space-y-1">
              ...
              {guestForm.formState.errors.lastname && (
                <p className="text-xs text-destructive">{guestForm.formState.errors.lastname.message}</p>
              )}
            </div>
          </div>
        ...
        <div className="py-6 space-y-6">
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm">Check your email for a confirmation.</p>
            </div>
          </div>
          ...
          <div className="rounded-xl border bg-card p-4 space-y-3">
            <div className="flex items-center gap-2">
              {confirmedSession.activityColor && (
                <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: confirmedSession.activityColor }} />
              )}
              <span className="font-semibold">{confirmedSession.activityName || 'Session'}</span>
            </div>
            <div className="text-sm space-y-1.5 text-muted-foreground">
              <p><span className="font-medium text-foreground">Date: </span>{formatDate(confirmedSession.start)}</p>
              <p><span className="font-medium text-foreground">Time: </span>{formatTime(confirmedSession.start)} – {formatTime(confirmedSession.end)}</p>
              {confirmedSession.instructorName && (
                <p><span className="font-medium text-foreground">Instructor: </span>{confirmedSession.instructorName}</p>
              )}
              {confirmedSession.location && (
                <p><span className="font-medium text-foreground">Location: </span>{confirmedSession.location}</p>
              )}
            </div>
          </div>
          ...
          {ctaUrl && (
            <a href={ctaUrl} target="_blank" rel="noopener noreferrer">
              <PortalButton accentColor={accentColor}>{ctaLabel}</PortalButton>
            </a>
          )}
