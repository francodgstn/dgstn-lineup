...
export default function BookingForm({ teamId, teamName, slug }: Props) {
        ...
        {!loadingSessions && sessions.length > 0 && (
          <div className="space-y-3">
            {sessions.map((s) => (
              <button
                key={s.id}
                onClick={() => { setSelectedSession(s); setStep('details') }}
      ...
      <div>
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Booking: <strong>{selectedSession.activityName || 'Session'}</strong> on {formatDateTime(selectedSession.start)}
          </p>
        </div>
  ...
  if (step === 'confirmed') {
    return (
      <div>
        {portalHeader}
        <div className="px-5 py-12 space-y-6 text-center">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Your <strong>{confirmedActivity}</strong> session at <strong>{teamName}</strong> has been booked.
              Check your email for a confirmation.
            </p>
          </div>
          <button
            onClick={() => { setStep('select'); setSelectedSession(null) }}
