...
type Step = 'email' | 'code' | 'form' | 'success'
...
const detailsSchema = z.object({
  ...
  privacyConsent: z.literal(true, { errorMap: () => ({ message: 'You must accept the privacy policy' }) }),
})
...
export default function MembershipSignupForm({ teamId, teamName, slug }: Props) {
  ...
  const onSubmitDetails = async (values: DetailsValues) => {
    ...
    try {
      const fn = httpsCallable<
        { codeId: string; contactDetails: Omit<DetailsValues, 'privacyConsent'> & { privacyConsent: boolean } },
        { success: boolean }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Enter your email address to get started. We&apos;ll send a verification code.
          </p>
        </div>
  ...
  if (step === 'form') {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Complete your details</h1>
          <p className="text-muted-foreground mt-1">
            Fill in your information to request membership at <strong>{teamName}</strong>.
          </p>
        </div>
        ...
        <form onSubmit={detailsForm.handleSubmit(onSubmitDetails)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              {detailsForm.formState.errors.firstname && (
                <p className="text-xs text-destructive">{detailsForm.formState.errors.firstname.message}</p>
              )}
            </div>
            <div className="space-y-1">
              ...
              {detailsForm.formState.errors.lastname && (
                <p className="text-xs text-destructive">{detailsForm.formState.errors.lastname.message}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-1">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-sm text-muted-foreground">
                I agree to the processing of my personal data for membership purposes at {teamName}.
              </span>
            </label>
            {detailsForm.formState.errors.privacyConsent && (
              <p className="text-xs text-destructive">{detailsForm.formState.errors.privacyConsent.message}</p>
            )}
          </div>
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50"
          >
            {detailsForm.formState.isSubmitting ? 'Submitting…' : 'Complete signup'}
          </button>
        </form>
      </div>
    )
    ...
    <div className="space-y-6 text-center py-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Your membership request has been received and is under review.
