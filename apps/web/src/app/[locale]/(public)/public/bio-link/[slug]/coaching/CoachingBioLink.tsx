...
function SlotCard({
  ...
  async function onVerifyCode(values: CodeValues) {
    ...
    try {
      ...
      const contactId =
        result.data.selectedContactId ?? result.data.matchedContacts?.[0]?.id ?? null
      if (!contactId) {
      ...
      if (e.code === 'already-exists') setError(t('errorAlreadyBooked'))
        ...
        setError(e.message || 'Members only. Please contact your coach.')
            ...
            <form onSubmit={handleSubmit(onSubmitGuest)} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label htmlFor={`fn-${slot.id}`} className="text-xs">
                    {t('fieldFirstname')}
                  </Label>
                ...
                <div className="space-y-1">
                  <Label htmlFor={`ln-${slot.id}`} className="text-xs">
                    {t('fieldLastname')}
                  </Label>
              ...
              <div className="space-y-1">
                <Label htmlFor={`em-${slot.id}`} className="text-xs">
                  {t('fieldEmail')}
                </Label>
              ...
              <div className="space-y-1">
                <Label htmlFor={`ph-${slot.id}`} className="text-xs">
                  {t('fieldPhone')}
                </Label>
