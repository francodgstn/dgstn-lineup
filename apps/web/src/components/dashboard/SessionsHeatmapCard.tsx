...
function buildGrid(
  ...
  if (source === 'participants') {
    ...
    for (const s of sessions) {
      const d = s.start?.toDate?.()
      if (d) add(d, s.portal_new_contact_bookings_count ?? 0)
    }
  } else {
...
interface Props {
  sessions?: SessionDoc[]; newContactBookings?: BookingDoc[]
  compareWith?: string
  comparisonSessions?: SessionDoc[]; comparisonNewContactBookings?: BookingDoc[]
  title?: string
}

export function SessionsHeatmapCard({
  sessions = [], newContactBookings = [], compareWith = 'none',
  comparisonSessions = [], comparisonNewContactBookings = [], title,
}: Props) {
  const [source, setSource] = useState('participants')
  const [autoHours, setAutoHours] = useState(false)

  const grid = useMemo(() => buildGrid(source, sessions, newContactBookings), [source, sessions, newContactBookings])
  const compGrid = useMemo(
    () => compareWith !== 'none' ? buildGrid(source, comparisonSessions, comparisonNewContactBookings) : null,
  ...
  const getCellBg = (val: number, compVal: number) => {
    ...
    const compColor    = compVal > 0 ? alpha(COMPARISON_COLOR, 0.15 + (compVal / maxVal) * 0.85) : empty
      ...
      <CardContent className="flex-1 flex flex-col pb-4 pt-3 overflow-x-auto">
        <div
          ...
          >
            ...
            <div />
            {DAYS.map((d) => (
              <span key={d} className="text-center text-[10px] font-bold text-muted-foreground">{d}</span>
            ))}
                      ...
                      <div key={`${hour}-${col}`} title={tooltipContent || undefined}>
                        {isComparing ? (
                          ...
                          <div
                            ...
                            style={{ height: CELL_SIZE, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                          >
                            {/* ghost dot */}
                            <div style={{
                              position: 'absolute', width: CELL_SIZE, height: CELL_SIZE,
                              borderRadius: '50%', background: alpha(PRIMARY_COLOR, 0.07),
                            }} />
                            {/* value dot */}
                            {dotSize > 0 && (
                              <div style={{
                                position: 'relative', width: dotSize, height: dotSize,
                                borderRadius: '50%', background: getCellBg(val, compVal),
                                overflow: 'hidden',
                              }} />
                            )}
                          </div>
        ...
        {isComparing && (
          <div className="flex items-center gap-4 justify-end mt-3">
            ...
            <div className="flex items-center gap-1">
              <div className="w-3.5 h-3.5 rounded-[3px] overflow-hidden"
                style={{ background: `linear-gradient(135deg, transparent 50%, ${alpha(PRIMARY_COLOR, 0.7)} 50%)` }} />
