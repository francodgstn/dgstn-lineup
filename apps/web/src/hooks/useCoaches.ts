...
// roster is the set of team members whose role is `coach` (same source as the
// /coaches page). Reads go through the `listTeamMembers` callable because member
// display names live on users/{uid}, which Firestore rules deny reading
// cross-user client-side.
//
// Fallback: teams without a coach roster (solo/free/coach plan) return every
// member as `pickable`, so a teaching owner can still be assigned. Studios with a
// coach roster get just the coaches.
...
export function useCoaches(teamId: string | null) {
  ...
  // Studios with a coach roster → just coaches; otherwise every member so a
  // solo owner/manager can still be picked as the instructor.
