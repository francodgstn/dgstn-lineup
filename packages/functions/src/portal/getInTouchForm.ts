...
// HTTP endpoint for the "get in touch" contact form on team portals.
...
async function resolveTeam(slug: string): Promise<{
  email: string; teamId: string; teamName: string; allowedOrigins: string[]
} | null> {
  ...
  if (notificationType === 'custom' && settings.teamEmail) {
    ...
    if (!ownerIdToFetch) {
      const snap = await db.collection('teams').doc(team.id!).collection('team_members').where('role', '==', 'owner').limit(1).get()
...
function buildNotificationEmail(fields: Record<string, string>, teamName: string): string {
  const rows = Object.entries(fields)
    .map(([key, val]) => `<tr><td style="padding:8px 12px;font-weight:600;color:#555;white-space:nowrap;vertical-align:top;">${key}</td><td style="padding:8px 12px;color:#333;">${val.replace(/\n/g, '<br>')}</td></tr>`)
    .join('')
...
export const getInTouchForm = onRequest(async (req, res) => {
  ...
  if (!team?.trim()) { res.status(400).json({ success: false, error: 'Missing required field: team' }); return }
  if (!fields || typeof fields !== 'object' || Array.isArray(fields)) {
    res.status(400).json({ success: false, error: 'fields must be a non-empty object' }); return
  }
  ...
  try {
    ...
    res.status(500).json({ success: false, error: 'Internal error' }); return
  }

  if (!resolved) { res.status(400).json({ success: false, error: 'Unknown team' }); return }

  const { allowedOrigins } = resolved
  if (allowedOrigins.length === 0) {
    res.status(403).json({ success: false, error: 'This team has not configured any allowed origins' }); return
  }
  ...
  if (origin && !allowedOrigins.includes(origin)) {
    res.status(403).json({ success: false, error: 'Origin not allowed' }); return
  }

  const sanitizedFields = sanitizeFields(fields as Record<string, unknown>)
  if (Object.keys(sanitizedFields).length === 0) {
    res.status(400).json({ success: false, error: 'fields must contain at least one value' }); return
  }
  ...
  if (!allowed) {
    res.status(429).json({ success: false, error: 'Too many submissions. Please try again later.' }); return
  }

  try {
    await admin.firestore()
      .collection('teams').doc(resolved.teamId)
    ...
    res.status(500).json({ success: false, error: 'Failed to store submission' }); return
  }
