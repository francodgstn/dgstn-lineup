...
export const moveContacts = onCall(async (request) => {
  ...
  if (!contactIds || contactIds.length === 0) throw new HttpsError('invalid-argument', 'contactIds is required')
  ...
  if (targetErr || !targetDoc || !targetDoc.exists) throw new HttpsError('not-found', 'Target team not found')
  ...
  for (const contactId of contactIds) {
    ...
    if (cErr || !contactDoc || !contactDoc.exists) throw new HttpsError('not-found', `Contact ${contactId} not found`)

    const data = contactDoc.data()!
    const contactTeamId = (data.teamId || data.teacher) as string | undefined
    if (!contactTeamId) throw new HttpsError('failed-precondition', `Contact ${contactId} has no team assigned`)
    if (!sourceTeamId) {
  ...
  if (!hasRole && !isLegacyOwner) throw new HttpsError('permission-denied', 'You must be a manager or owner of the source team')

  const hasTargetRole = await hasTeamRole(authId, newTeamId, 'manager')
  if (!hasTargetRole && authId !== newTeamId) throw new HttpsError('permission-denied', 'You must be a manager or owner of the target team')
