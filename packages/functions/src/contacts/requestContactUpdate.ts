...
//   2. codeId     — from the portal email-verification flow
...
export const requestContactUpdate = onCall(async (request) => {
  ...
  if (!codeId && !authToken) throw new HttpsError('invalid-argument', 'Missing required field: codeId or authToken')
  if (!contactDetails) throw new HttpsError('invalid-argument', 'Missing required field: contactDetails')
  ...
  if (authToken) {
    ...
    if (tokenData.type !== 'membership') throw new HttpsError('permission-denied', 'Token is not a membership token')
    ...
    if (request.data.teamId && tokenData.team_id !== request.data.teamId) throw new HttpsError('permission-denied', 'Token does not match the requested team')
    ...
    // ── Code-based auth (portal email verification) ───────────────────────────
    ...
    if (!codeData.verified) throw new HttpsError('failed-precondition', 'Email not verified. Please verify your email first.')
    if (codeData.used) throw new HttpsError('already-exists', 'This verification code has already been used')

    const verifiedAt = codeData.verifiedAt as Timestamp
    const thirtyMinutesAgo = Timestamp.fromMillis(Date.now() - 30 * 60 * 1000)
    if (verifiedAt.toMillis() < thirtyMinutesAgo.toMillis()) throw new HttpsError('deadline-exceeded', 'Verification expired. Please start over.')
  ...
  if ((contactDoc.data()!.teamId as string) !== teamId) throw new HttpsError('permission-denied', 'Contact does not belong to this team')

  // ── Duplicate check ─────────────────────────────────────────────────────────
  const existingSnap = await db
    .collection('teams').doc(teamId).collection(CONTACT_REQUESTS_SUBCOLLECTION)
    .where('contact_id', '==', contactId).where('status', '==', 'pending').limit(1).get()
  if (!existingSnap.empty) throw new HttpsError('already-exists', 'A pending update request already exists for this contact')
  ...
  const sanitizedData: Record<string, unknown> = {
    ...
    birthdate: contactDetails.birthdate ? Timestamp.fromDate(new Date(contactDetails.birthdate as string)) : null,
  ...
  // ── Create request ──────────────────────────────────────────────────────────
  const requestRef = db.collection('teams').doc(teamId).collection(CONTACT_REQUESTS_SUBCOLLECTION).doc()
  const contactData = contactDoc.data()!
  const contactName = `${(contactData.firstname as string) || ''} ${(contactData.lastname as string) || ''}`.trim()
  ...
  // ── Team alert ──────────────────────────────────────────────────────────────
  try {
    await db.collection('teams').doc(teamId).collection('team_alerts').doc().set({
      teamId,
  ...
  // ── Mark auth as used ───────────────────────────────────────────────────────
  if (tokenRef) {
    await tokenRef.update({ used: true, used_at: FieldValue.serverTimestamp(), used_for_request: requestRef.id })
  }
  if (codeRef) {
