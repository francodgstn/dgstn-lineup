...
export async function seedAccounts(
  ...
  for (const acc of template.accounts) {
    try {
      await accountsCollection(teamId).doc(acc.code).create({
        code: acc.code,
...
/**
 * Ensure the team has accounting settings + a seeded chart. Template comes
 * from team.country (CH fallback); account names from team.language. Returns
 * the (possibly just-created) settings.
 */
export async function ensureAccountingSeeded(teamId: string): Promise<AccountingSettings> {
  ...
  if (existing) {
    // Accounts are create-only — safe to re-run for reinstalls that kept settings.
  ...
  console.log(`[accounting] seeded team=${teamId} template=${templateId} accounts=${created}`)
