 ...
 *
 * So exactly one contact address may be given a KNOWN code, which is never
 * mailed. `sendContactVerificationCode` is the only place a contact code is
 ...
 *
 * - **One address**, matched exactly after normalisation.
...
export interface ReviewAccess {
  enabled: boolean
  /** The one contact email this applies to. */
  email: string
...
export function decideReviewCode(
  ...
  if (typeof access.email !== 'string' || access.email.toLowerCase().trim() !== email) return null
...
export async function reviewAccessCodeFor(
  ...
  if (!code && access?.enabled === true && access.email?.toLowerCase().trim() === email) {
    ...
    console.warn(`[review-otp] access for ${email} is configured but not usable (expired, or a malformed code)`) // eslint-disable-line no-console
  }
  return code
}
