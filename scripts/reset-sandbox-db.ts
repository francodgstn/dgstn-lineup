 ...
 *   pnpm reset:sandbox              # has --confirm wired in package.json
 *   tsx scripts/reset-sandbox-db.ts --confirm
 *
 * Safety:
 *   - Hard-codes the project to `linyup-sandbox`. It refuses to run against any
 *     other project id, so it can never be pointed at staging/production.
...
if (!values.confirm) {
  ...
  console.error('   Re-run:  pnpm reset:sandbox   (or add --confirm)')
  process.exit(1)
}
